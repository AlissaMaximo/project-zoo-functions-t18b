...
function getRelatedEmployees(managerId) {
  ...
  if (isManager(managerId) === true) {
    const collaborators = data.employees
      .filter((empl) => empl.managers.find((mgr) => mgr === managerId) === managerId);
    const collaboratorsNames = [];
    const collaboratorsFirstNames = collaborators.map((collaborator) => collaborator.firstName);
    const collaboratorsLastNames = collaborators.map((collaborator) => collaborator.lastName);
    const lengthIndex = collaboratorsFirstNames.length;
